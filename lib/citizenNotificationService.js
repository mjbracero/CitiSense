import { sendRolePushNotification } from "./notificationPush";
import { supabase } from "./supabase";

export async function createCitizenNotificationAndPush({
  citizenId,
  complaintId,
  shortId,
  type = "status",
  title,
  message,
  status,
  metadata = {},
}) {
  if (!citizenId || !complaintId) {
    return {
      success: false,
      error: new Error("Missing citizen or complaint id."),
    };
  }

  const { data, error } = await supabase
    .from("complaint_notifications")
    .insert({
      citizen_id: citizenId,
      complaint_id: complaintId,
      type,
      title,
      message,
      status,
      is_read: false,
      metadata: {
        short_id: shortId,
        ...metadata,
      },
    })
    .select("id, citizen_id, complaint_id, title, message")
    .single();

  if (error) {
    console.log("Create citizen notification error:", error);
    return { success: false, error };
  }

  try {
    const pushResult = await sendRolePushNotification({
      userId: citizenId,
      title: title || "Complaint Update",
      body: message || "Your complaint has a new update.",
      role: "citizen",
      complaintId,
      notificationId: data.id,
    });

    if (!pushResult.pushSent) {
      return {
        success: true,
        notification: data,
        pushSent: false,
        pushError: pushResult.pushError,
      };
    }

    return { success: true, notification: data, pushSent: true };
  } catch (pushError) {
    console.log("Citizen push invoke catch:", pushError);
    return { success: true, notification: data, pushSent: false, pushError };
  }
}

export async function notifyCitizenDuplicateSubmission({
  citizenId,
  complaintId,
  shortId,
  duplicateReason,
  similarComplaintId = null,
}) {
  const complaintLabel = shortId || complaintId;
  const notificationTitle = "Similar Complaint Detected";
  const notificationMessage =
    duplicateReason ||
    `Your complaint #${complaintLabel} looks similar to an active report you already submitted. We saved it, but you may want to track your existing complaint instead.`;

  return createCitizenNotificationAndPush({
    citizenId,
    complaintId,
    shortId: complaintLabel,
    type: "duplicate",
    title: notificationTitle,
    message: notificationMessage,
    status: "Pending",
    metadata: {
      duplicate_reason: duplicateReason || null,
      similar_complaint_id: similarComplaintId,
    },
  });
}

export async function notifyCitizenComplaintStatusChange({
  citizenId,
  complaintId,
  shortId,
  oldStatus,
  newStatus,
  updatedBy,
  department,
  category,
  title: complaintTitle,
}) {
  const notificationTitle = "Complaint Status Updated";
  const notificationMessage = `Your complaint #${shortId} status changed from ${oldStatus} to ${newStatus}.`;

  return createCitizenNotificationAndPush({
    citizenId,
    complaintId,
    shortId,
    type: "status",
    title: notificationTitle,
    message: notificationMessage,
    status: newStatus,
    metadata: {
      old_status: oldStatus,
      new_status: newStatus,
      updated_by: updatedBy || null,
      assigned_office: department || null,
      title: complaintTitle || null,
      category: category || null,
    },
  });
}

export async function notifyCitizenAiValidationResult({
  citizenId,
  complaintId,
  shortId,
  approved,
  reason,
  summary,
}) {
  const complaintLabel = shortId || complaintId;
  const isApproved = Boolean(approved);
  const cleanReason = String(reason || "").trim();
  const notificationTitle = isApproved
    ? "Validation Approved"
    : "Validation Not Approved";
  const notificationMessage = isApproved
    ? `AI approved your validation for complaint #${complaintLabel}. An admin will review it before completing the complaint.`
    : `AI did not approve your validation for complaint #${complaintLabel}.${
        cleanReason ? ` ${cleanReason}` : ""
      } An admin will review it next.`;

  return createCitizenNotificationAndPush({
    citizenId,
    complaintId,
    shortId: complaintLabel,
    type: "ai_validation",
    title: notificationTitle,
    message: notificationMessage,
    status: isApproved ? "Approved" : "Rejected",
    metadata: {
      ai_status: isApproved ? "approved" : "rejected",
      ai_reason: cleanReason || null,
      ai_summary: summary || null,
    },
  });
}
