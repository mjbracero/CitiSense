...
const CATEGORY_KEYWORDS = [
  ...
  },
  {
    ...
    keywords: ["drainage", "flood", "flooding", "flooded", "baha", "canal", "sewer"],
  },
  {
  ...
  },
  {
    ...
    keywords: ["stray dog", "stray cat", "dog bite", "rabies", "livestock", "veterinary"],
  },
  {
...
/** Categories that should override AI when keyword evidence is present. */
...
/**
 * Prefer keyword routing when AI picks a catch-all (e.g. City Facility / GSO)
 * but the complaint text clearly belongs to a specific LGU category.
 */
export function reconcileComplaintCategory(title = "", description = "", aiCategory = "") {
  ...
  if (
    normalizedAi === "City Facility Concerns" &&
...
function existingOfficeMatchesCategory(existingOffice, category) {
  const categoryOffices =
    DEPARTMENTS_BY_CATEGORY[normalizeComplaintCategory(category)] || [];
  const cleanExisting = String(existingOffice || "").replace(/\s+/g, " ").trim();

  if (!cleanExisting || normalizeOfficeKey(cleanExisting) === "unassigned") {
    return false;
  }

  const existingCanonical = parseAssignedOffices(cleanExisting)
    .map((part) => findCanonicalOffice(part))
    .filter(Boolean);

  if (!existingCanonical.length || !categoryOffices.length) {
    return false;
  }

  return existingCanonical.every((office) =>
...
/** Derive category and assigned office from complaint text and optional AI category. */
export function resolveComplaintRouting(
  ...
  const preserveExisting = existingOfficeMatchesCategory(
    existingOffice,
    category
  );
  ...
  return {
    category,
    assignedOffice: getAssignedOffice(
      category,
      preserveExisting ? existingOffice : null
    ),
  };
}
