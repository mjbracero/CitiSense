...
export async function getProfileDisplayName(userId) {
  ...
  if (error) {
    console.log("Load profile display name error:", error);
    return "Citizen";
  }

  return data?.full_name || data?.email || "Citizen";
}
...
export async function notifyAllAdmins({
  ...
  try {
    const { data, error } = await supabase.functions.invoke(
      "notify-admins-citizen-validated",
      {
        body: {
          complaintId,
          type,
    ...
    if (error) {
      console.log("Notify admins edge error:", error);
    } else if (data?.error) {
      console.log("Notify admins edge response:", data);
    }
  } catch (edgeError) {
    console.log("Notify admins edge catch:", edgeError);
  }
...
export async function notifyAdminsCitizenValidated({
  ...
  const payload = {
    ...
    complaint: {
      ...
      category: complaint.category,
      assigned_office: department,
      location_text: complaint.location_text || complaint.location,
  ...
  try {
    ...
    if (error) {
      console.log("Notify admins citizen validated edge error:", error);
    } else if (data?.error) {
      console.log("Notify admins citizen validated edge response:", data);
    }
  } catch (edgeError) {
    console.log("Notify admins citizen validated edge catch:", edgeError);
  }
  ...
  return notifyAllAdmins({
    ...
    category: complaint.category,
    department,
    locationText: complaint.location_text || complaint.location,
    metadata: {
      short_id: shortId,
      complaint_title: complaint.title,
      title: complaint.title,
