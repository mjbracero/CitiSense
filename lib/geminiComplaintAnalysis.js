...
- Read the full meaning of the title and description in any Philippine language — do not rely on English keywords alone.
- Classify into the most specific LGU category from the allowed list.
- Use the department routing map above — each category routes to its listed office(s) only.
- Prefer a specific service category over "City Facility Concerns" whenever the complaint is about permits, certificates, taxes, utilities, roads, traffic, health, waste, or similar LGU services.
- Do NOT use "City Facility Concerns" as a catch-all.
- Business permit, mayor's permit, licensing, and permit processing delays -> "Business Permit and Licensing Concerns".
- Birth/marriage/death certificates, civil registry, PSA records -> "Civil Registry Concerns".
- Real property tax, treasury payments, receipts -> "Tax and Treasury Concerns".
- Property valuation, tax declaration, assessor -> "Property Assessment Concerns".
- Port / Polambato / ferry -> "Port Concerns".
- Bus/jeepney/van terminal -> "Transport Terminal Concerns".
- PWD / wheelchair / accessibility -> "PWD Accessibility Concerns".
- Coastal / Bantay Dagat / illegal fishing -> "Coastal and Marine Protection Concerns".
- "City Facility Concerns" is ONLY for physical city-owned buildings and facilities (city hall, gym, covered court, waiting areas, public restrooms, sports complexes).
- Set confidence higher when the category is clear from meaning; lower when ambiguous.
...
export function normalizeGeminiAnalysis(rawAnalysis, fallbackInput = {}) {
  ...
  const routing = resolveComplaintRouting(
    title,
    description,
    rawAnalysis?.category || fallbackCategory,
    null,
    { confidence: rawAnalysis?.confidence }
  );
  const { category, assignedOffice } = routing;
...
export function buildKeywordFallbackAnalysis({
  ...
  const { category, assignedOffice } = resolveComplaintRouting(
    title,
    description,
    detectComplaintCategoryFromKeywords(title, description)
  );
  const is_emergency =
...
export async function analyzeComplaintWithGemini({
  ...
  // Non-emergency: full AI analysis with more photo evidence.
  // Emergency: leaner payload for faster response, still AI-powered.
  const maxPhotos = isEmergency ? 1 : 3;
  const timeoutMs = isEmergency ? 14000 : 28000;
  const maxOutputTokens = isEmergency ? 1100 : 1800;
