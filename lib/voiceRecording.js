...
/**
 * HIGH_QUALITY-style m4a — Expo Go metering is unreliable with WAV/PCM.
 * Keep mono 16k for smaller uploads while preserving metering.
 */
...
async function createPreparedRecording(onStatusUpdate) {
  return withRecorderLock(async () => {
    ...
    try {
      ...
      if (!/only one recording/i.test(message)) {
        throw error;
      }

      // Native singleton still held — force another release and retry once.
      await releaseGlobalRecording();
      await wait(250);
